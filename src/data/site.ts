...
export const PRIMARY_NAV: NavItem[] = [
  {
    ...
    children: [
      { label: 'All cost pages', href: '/heat-pump-cost/', description: 'Installed cost by system type and region' },
  ...
  },
  {
    ...
    children: [
      ...
      { label: 'Mass Save', href: '/rebates/mass-save/', description: 'Massachusetts, up to $10,000' },
      { label: 'Efficiency Maine', href: '/rebates/efficiency-maine/', description: 'Income tiers and reserved HEAR money' },
      { label: 'NYSERDA', href: '/rebates/nys-clean-heat/', description: 'NYS Clean Heat plus New York HEAR' },
      { label: 'Efficiency Vermont', href: '/rebates/efficiency-vermont/', description: 'Open rebate, waitlisted HEAR' },
...
export function isNavItemActive(item: NavItem, current: string): boolean {
  if (current === item.href) return true;
  if (item.children?.some((child) => current === child.href || current.startsWith(child.href))) {
    return true;
  }
  return !item.children && item.href !== '/' && current.startsWith(item.href);
}
...
export const FOOTER_NAV: { title: string; links: NavItem[] }[] = [
  ...
  },
  {
    ...
    links: [
      ...
      { label: 'Sources', href: '/methodology/#sources' },
